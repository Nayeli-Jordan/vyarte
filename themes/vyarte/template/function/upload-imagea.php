...
$filename   = 'dsgn_' . $currentDate . $nombre_imagea; /* Nombramos la imagen con el nombre del archivo  */
$uploadfile = $upload_dir["path"] . "/" . $filename; /* Guardamos la ruta del archivo temporal */

if ( move_uploaded_file( $_FILES['persImagea']['tmp_name'], $uploadfile ) ) {
  ...
  $imagea = wp_get_attachment_url( $attach_id );
  update_post_meta($my_post_id, 'vy_personalizado_imagea', $imagea);
} ?>